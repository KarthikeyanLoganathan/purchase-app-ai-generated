...
function onOpen() {
    ...
    // Save to a hidden "Metadata" sheet or Developer Metadata
    sheet.addDeveloperMetadata('theGoogleAppScriptId', scriptId);    
    ...
    ui.createMenu("Purchase App")
        ...
        .addSeparator()
        .addItem("Export Current Sheet to CSV", `exportImport.${exportImport.csvExportCurrentSheet.name}`)
        .addItem("Export All Data Sheets to ZIP", `exportImport.${exportImport.csvExportAllDataSheets.name}`)
        .addItem("Import All Data Sheets from ZIP", `exportImport.${exportImport.csvImportAllDataSheets.name}`)
        .addSeparator()
        ...
        .addItem("Cleanup All Data Sheets", `cleanup.${cleanup.cleanupAllDataSheets.name}`)
        .addSeparator()
