...
class ChangeLogUtils {
  ...
  Future<String> initializeChangeLogFromDataTables() async {
    ...
    final syncTables = SyncConfig.getSyncTables();

    for (final tableName in syncTables) {
      final tableIndex = SyncConfig.getTableIndex(tableName);
      if (tableIndex == null) {
        debugPrint('WARNING: No table index for $tableName, skipping');
        continue;
      }

      final keyColumn = TableKeyColumns.getKeyColumn(tableName);
        ...
        for (final record in records) {
          ...
          if (tableKey != null && tableKey.toString().trim().isNotEmpty) {
            changeLogRecords.add({
              ...
              'change_mode': SyncConfig.changeModeInsert,
  ...
  Future<List<Map<String, dynamic>>> prepareCondensedChangeLogFromChangeLog(
    ...
    for (final row in changeLogData) {
      ...
      if (changeMode == SyncConfig.changeModeInsert ||
          changeMode == SyncConfig.changeModeUpdate) {
      ...
      } else if (changeMode == SyncConfig.changeModeDelete) {
        ...
        if (oldRec != null &&
            (oldRec['change_mode'] == SyncConfig.changeModeInsert ||
                oldRec['change_mode'] == SyncConfig.changeModeUpdate)) {
          ...
          tableChangeHistory[changeHistoryTableMemberName]!.remove(tableKey);
            ...
            oldRec['change_mode'] == SyncConfig.changeModeDelete) {
    ...
    try {
      ...
      for (final rec in records) {
        batch.insert(TableNames.changeLog, {
          ...
          'change_mode': rec['change_mode'] ?? SyncConfig.changeModeInsert,
