...
class _ManufacturerMaterialDetailScreenState
  ...
  void initState() {
    ...
    // Load currency first to get correct decimal places
    if (widget.manufacturerMaterial.currency != null &&
        widget.manufacturerMaterial.currency!.isNotEmpty) {
      _currencyController.text = widget.manufacturerMaterial.currency!;
    }
    _maxRetailPriceController.text = widget.manufacturerMaterial.maxRetailPrice
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return FutureBuilder<Vendor?>(
                          ...
                          builder: (context, snapshot) {
                            ...
                            final vplDecimalPlaces = SettingsManager.instance
                                .getCurrencyDecimalPlaces(vpl.currency ?? '');
                            return Card(
                              margin: const EdgeInsets.only(bottom: 8),
                              child: ListTile(
                                title: Text(vendor?.name ?? 'Unknown Vendor'),
                                subtitle: Text(
                                  'Rate: ${vpl.rate.toStringAsFixed(vplDecimalPlaces)} ${vpl.currency ?? ''} | Tax: ${vpl.taxPercent.toStringAsFixed(SettingsManager.instance.getPercentDecimalPlaces())}%',
                                ),
                                trailing: IconButton(
