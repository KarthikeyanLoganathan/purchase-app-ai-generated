...
import "../services/csv_export_import_service.dart";
...
class _SetupGoogleSheetScreenState extends State<SetupGoogleSheetScreen> {
    ...
    try {
      ...
      if (spreadsheetId != null && spreadsheetId.isNotEmpty) {
        ...
        if (needsNewSheet) {
          ...
          setState(() =>
              _statusMessage = "Previous sheet in trash, creating new one...");
  ...
  Future<void> _setupSheets() async {
    if (_deployedWebAppUrl == null || _currentSpreadsheetId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
  ...
  Future<void> _importSampleData() async {
    ...
    try {
      final importService = CsvExportImportService();
  ...
  String _columnIndexToLetter(int columnIndex) {
    String letter = '';
    int num = columnIndex;
  ...
  Future<void> _uploadDataToGoogleSheets() async {
    ...
    try {
      ...
      for (final tableName in TableNames.allDataTables) {
        try {
          setState(() {
            _statusMessage = 'Processing table: $tableName...';
          });
          ...
          final tableColumnNames =
              await dbHelper.getTableColumnNames(tableName);
          debugPrint('[Upload] Table columns: $tableColumnNames');

          // Get Google Sheet worksheet by name
          final spreadsheet =
              await sheetsApi.spreadsheets.get(_currentSpreadsheetId!);
          ...
          debugPrint('[Upload] Sheet columns: $sheetColumnNames');
          ...
          if (maxRows > 1) {
            ...
            final numCols = sheetColumnNames.length;
            final lastColLetter =
                _columnIndexToLetter(numCols - 1); // 0-based index
          ...
          while (true) {
            ...
            for (final row in records) {
              ...
              for (final sheetCol in sheetColumnNames) {
                ...
                final dbCol = tableColumnNames.firstWhere(
                  (col) => col.toLowerCase() == sheetCol.toLowerCase(),
                  orElse: () => sheetCol,
                );

                final value = row[dbCol];
                excelRow.add(value ?? '');
              }
          ...
          debugPrint(
              '[Upload] Completed $tableName: $recordsProcessed records');
          totalTablesProcessed++;
        } catch (e) {
          debugPrint('[Upload] Error processing table $tableName: $e');
      ...
      debugPrint('[Upload] Getting last sync timestamp from change_log...');
      final lastSyncResult = await db.rawQuery(
        'SELECT MAX(updated_at) as max_updated_at FROM ${TableNames.changeLog}',
      );
      final lastSyncTimestamp = lastSyncResult.isNotEmpty
          ? lastSyncResult.first['max_updated_at'] as String?
          : null;
    ...
    try {
      ...
      final jsFiles = allAssets
          .where((key) =>
              ...
              (key.endsWith(".js") || key.endsWith(".html")))
          .toList();
      ...
      // Add appsscript.json manifest file
  ...
  Future<String> _deployWebApp(String scriptId) async {
    ...
    try {
      ...
      debugPrint("[Setup] Checking for existing deployments...");
      final deploymentsList =
          await scriptApi.projects.deployments.list(scriptId);

      script.Deployment? existingWebAppDeployment;

      if (deploymentsList.deployments != null &&
          deploymentsList.deployments!.isNotEmpty) {
        // Find existing WEB_APP deployment
        for (var deployment in deploymentsList.deployments!) {
          if (deployment.entryPoints != null) {
            final hasWebApp = deployment.entryPoints!
                .any((entry) => entry.entryPointType == "WEB_APP");
            if (hasWebApp) {
              existingWebAppDeployment = deployment;
              debugPrint(
                  "[Setup] Found existing Web App deployment: ${deployment.deploymentId}");
              break;
            }
          }
        }
      ...
      if (existingWebAppDeployment != null) {
        // Delete the old deployment since we don't need URL stability
        // This avoids the "Read-only deployments may not be modified" error
        debugPrint(
            "[Setup] Deleting old deployment: ${existingWebAppDeployment.deploymentId}");

        try {
          await scriptApi.projects.deployments.delete(
            scriptId,
            existingWebAppDeployment.deploymentId!,
          );
          debugPrint("[Setup] Old deployment deleted successfully");
        } catch (e) {
          debugPrint("[Setup] Warning: Could not delete old deployment: $e");
          // Continue anyway - we'll create a new one
        }
      }

      // Always create a new deployment with the latest version
      debugPrint(
                ...
                for (var metadata in spreadsheet.developerMetadata!) {
                  if (metadata.metadataKey ==
                    ...
                    try {
                      ...
                      debugPrint("[Setup] Error fetching script info: $e");
                    }
                    break;
                  }
                }
      ...
      try {
        ...
        try {
          ...
          if (spreadsheet.developerMetadata != null) {
            for (var metadata in spreadsheet.developerMetadata!) {
              if (metadata.metadataKey == SheetMetadata.theGoogleAppScriptId) {
                newScriptId = metadata.metadataValue!;
                debugPrint("[Setup] Found script ID: $newScriptId");

                // Get script project name
                final scriptApi = script.ScriptApi(newClient);
                try {
                  final project = await scriptApi.projects.get(newScriptId);
          ...
          if (newScriptId == null) {
            debugPrint("[Setup] No script ID found in developer metadata");
          }
        } catch (e) {
          debugPrint("[Setup] Error reading developer metadata: $e");
        }
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SelectableText(
                          ...
                          "5. You may see a blank page or error - that's OK\n"
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton.icon(
                            ...
                            icon: const Icon(Icons.folder_open,
                                color: Colors.white),
                            label: const Text(
                              "Import Sample Data from Assets",
                              style:
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 12),
                      const SelectableText(
                        "1. Complete Step 1 first (enable Apps Script API)\n"
                        "2. Sign in with your Google account\n"
                        "3. Enter a name for your Google Sheet\n"
                        "4. Create a secure app code (at least 8 characters)\n"
                        "5. Click \"Setup Google Sheets\"\n"
