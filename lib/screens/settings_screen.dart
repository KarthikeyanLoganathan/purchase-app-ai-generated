...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _exportData() async {
    try {
      ...
      final exportService = CsvExportImportService();

      // Get export summary first
      final summary = await exportService.getExportSummary();

      // Export all tables to zip
      final zipFilePath = await exportService.exportAllTablesToZip();
      ...
      if (mounted) {
        ...
        if (action == 'share') {
          await exportService.shareExportedZip(zipFilePath);
        } else if (action == 'save') {
          try {
            final savedPath = await exportService.saveToDownloads(zipFilePath);
            if (mounted) {
