...
class DatabaseHelper {
  ...
  Future<int> insertManufacturer(Manufacturer manufacturer) async {
    ...
    await logChange(TableNames.manufacturers, manufacturer.uuid,
        SyncConfig.changeModeInsert);
    return result;
  }
  ...
  Future<int> deleteManufacturer(String uuid) async {
    ...
    if (result > 0) {
      await logChange(
          TableNames.manufacturers, uuid, SyncConfig.changeModeDelete);
    }
  ...
  Future<int> insertVendor(Vendor vendor) async {
    ...
    // Log change for delta sync
    await logChange(
        TableNames.vendors, vendor.uuid, SyncConfig.changeModeInsert);
  ...
  Future<int> updateVendor(Vendor vendor) async {
    ...
    if (result > 0) {
      await logChange(
          TableNames.vendors, vendor.uuid, SyncConfig.changeModeUpdate);
    }
  ...
  Future<int> deleteVendor(String uuid) async {
    ...
    // Log change for delta sync
    if (result > 0) {
      await logChange(TableNames.vendors, uuid, SyncConfig.changeModeDelete);
    }
  ...
  Future<int> insertProject(Project project) async {
    final db = await database;
    final result = await db.insert(TableNames.projects, project.toMap());

    // Log change for delta sync
    await logChange(
        TableNames.projects, project.uuid, SyncConfig.changeModeInsert);
  ...
  Future<int> insertMaterial(Material material) async {
    ...
    // Log change for delta sync
    await logChange(
        TableNames.materials, material.uuid, SyncConfig.changeModeInsert);
  ...
  Future<int> updateMaterial(Material material) async {
    ...
    if (result > 0) {
      await logChange(
          TableNames.materials, material.uuid, SyncConfig.changeModeUpdate);
    }
  ...
  Future<int> deleteMaterial(String uuid) async {
    ...
    // Log change for delta sync
    if (result > 0) {
      await logChange(TableNames.materials, uuid, SyncConfig.changeModeDelete);
    }
  ...
  Future<int> insertManufacturerMaterial(ManufacturerMaterial mm) async {
    final db = await database;
    final result =
        await db.insert(TableNames.manufacturerMaterials, mm.toMap());

    // Log change for delta sync
    await logChange(
        TableNames.manufacturerMaterials, mm.uuid, SyncConfig.changeModeInsert);
    return result;
  }
  ...
  Future<int> updateManufacturerMaterial(ManufacturerMaterial mm) async {
    ...
    // Log change for delta sync
    if (result > 0) {
      await logChange(TableNames.manufacturerMaterials, mm.uuid,
          SyncConfig.changeModeUpdate);
    }
  ...
  Future<int> deleteManufacturerMaterial(String uuid) async {
    ...
    if (result > 0) {
      await logChange(
          TableNames.manufacturerMaterials, uuid, SyncConfig.changeModeDelete);
    }
  ...
  Future<int> insertVendorPriceList(VendorPriceList vpl) async {
    final db = await database;
    final result = await db.insert(TableNames.vendorPriceLists, vpl.toMap());

    // Log change for delta sync
    await logChange(
        TableNames.vendorPriceLists, vpl.uuid, SyncConfig.changeModeInsert);
    return result;
  }
  ...
  Future<int> updateVendorPriceList(VendorPriceList vpl) async {
    ...
    if (result > 0) {
      await logChange(
          TableNames.vendorPriceLists, vpl.uuid, SyncConfig.changeModeUpdate);
    }
  ...
  Future<int> deleteVendorPriceList(String uuid) async {
    ...
    if (result > 0) {
      await logChange(
          TableNames.vendorPriceLists, uuid, SyncConfig.changeModeDelete);
    }
  ...
  Future<int> insertPurchaseOrder(PurchaseOrder po) async {
    ...
    // Log change for delta sync
    await logChange(
        TableNames.purchaseOrders, po.uuid, SyncConfig.changeModeInsert);
    return nextId;
  }
  ...
  Future<int> updatePurchaseOrder(PurchaseOrder po) async {
    ...
    if (result > 0) {
      await logChange(
          TableNames.purchaseOrders, po.uuid, SyncConfig.changeModeUpdate);
    }
  ...
  Future<int> deletePurchaseOrder(String uuid) async {
    ...
    for (final payment in payments) {
      ...
      await logChange(TableNames.purchaseOrderPayments,
          payment['uuid'] as String, SyncConfig.changeModeDelete);
    }
    ...
    if (result > 0) {
      await logChange(
          TableNames.purchaseOrders, uuid, SyncConfig.changeModeDelete);
    }
  ...
  Future<int> insertPurchaseOrderItem(PurchaseOrderItem poi) async {
    final db = await database;
    final result = await db.insert(TableNames.purchaseOrderItems, poi.toMap());
    // Log change for delta sync
    await logChange(
        TableNames.purchaseOrderItems, poi.uuid, SyncConfig.changeModeInsert);
  ...
  Future<int> updatePurchaseOrderItem(PurchaseOrderItem poi) async {
    ...
    if (result > 0) {
      await logChange(
          TableNames.purchaseOrderItems, poi.uuid, SyncConfig.changeModeUpdate);
    }
  ...
  Future<int> deletePurchaseOrderItem(String uuid) async {
    ...
    if (result > 0) {
      await logChange(
          TableNames.purchaseOrderItems, uuid, SyncConfig.changeModeDelete);
    }
  ...
  Future<int> insertPurchaseOrderPayment(PurchaseOrderPayment payment) async {
    final db = await database;
    final result =
        await db.insert(TableNames.purchaseOrderPayments, payment.toMap());
    // Log change for delta sync
    await logChange(TableNames.purchaseOrderPayments, payment.uuid,
        SyncConfig.changeModeInsert);
  ...
  Future<int> deletePurchaseOrderPayment(String uuid) async {
    ...
    if (result > 0) {
      await logChange(
          TableNames.purchaseOrderPayments, uuid, SyncConfig.changeModeDelete);
    }
  ...
  Future<int> insertBasket(Basket basket) async {
    ...
    final result = await db.insert(TableNames.baskets, basketWithId.toMap());

    // Log change for delta sync
    await logChange(
        TableNames.baskets, basket.uuid, SyncConfig.changeModeInsert);
  ...
  Future<List<Basket>> getAllBaskets() async {
    ...
    return result.map((map) => Basket.fromMap(map)).toList();
  }
  ...
  Future<Basket?> getBasket(String uuid) async {
    ...
    if (result.isEmpty) return null;
    return Basket.fromMap(result.first);
  }
  ...
  Future<int> updateBasket(Basket basket, {bool logChange = true}) async {
    ...
    if (result > 0 && logChange) {
      await this.logChange(
          TableNames.baskets, basket.uuid, SyncConfig.changeModeUpdate);
    }
  ...
  Future<int> deleteBasket(String uuid) async {
    ...
    // Log change for delta sync
    if (result > 0) {
      await logChange(TableNames.baskets, uuid, SyncConfig.changeModeDelete);
    }
  ...
  Future<int> insertBasketItem(BasketItem item) async {
    ...
    final result = await db.insert(TableNames.basketItems, itemWithId.toMap());

    // Log change for delta sync
    await logChange(
        TableNames.basketItems, item.uuid, SyncConfig.changeModeInsert);
  ...
  Future<int> updateBasketItem(BasketItem item) async {
    ...
    if (result > 0) {
      await logChange(
          TableNames.basketItems, item.uuid, SyncConfig.changeModeUpdate);
  ...
  Future<int> deleteBasketItem(String uuid) async {
    ...
    if (result > 0) {
      await logChange(
          TableNames.basketItems, uuid, SyncConfig.changeModeDelete);
  ...
  Future<int> insertQuotation(Quotation vendor) async {
    ...
    final result = await db.insert(TableNames.quotations, vendorWithId.toMap());

    // Log change for delta sync
    await logChange(
        TableNames.quotations, vendor.uuid, SyncConfig.changeModeInsert);
  ...
  Future<int> updateQuotation(Quotation vendor) async {
    ...
    if (result > 0) {
      await logChange(
          TableNames.quotations, vendor.uuid, SyncConfig.changeModeUpdate);
    }
  ...
  Future<int> deleteQuotation(String uuid) async {
    ...
    // Log change for delta sync
    if (result > 0) {
      await logChange(TableNames.quotations, uuid, SyncConfig.changeModeDelete);
    }
  ...
  Future<int> insertQuotationItem(QuotationItem item) async {
    final db = await database;
    final result = await db.insert(TableNames.quotationItems, item.toMap());
    // Log change for delta sync
    await logChange(
        TableNames.quotationItems, item.uuid, SyncConfig.changeModeInsert);
  ...
  Future<int> updateQuotationItem(QuotationItem item) async {
    ...
    if (result > 0) {
      await logChange(
          TableNames.quotationItems, item.uuid, SyncConfig.changeModeUpdate);
      // Update basket vendor totals
      await _updateQuotationTotals(item.quotationUuid);
    }
  ...
  Future<int> deleteQuotationItem(String uuid) async {
    ...
    if (result > 0) {
      await logChange(
          TableNames.quotationItems, uuid, SyncConfig.changeModeDelete);
  ...
  Future<void> _createVendorItemForBasketItem(
    ...
    await db.insert(TableNames.quotationItems, item.toMap());
    await logChange(
        TableNames.quotationItems, item.uuid, SyncConfig.changeModeInsert);
  ...
  Future<void> _updateVendorItemForBasketItem(
    ...
    await logChange(TableNames.quotationItems, vendorItem.uuid,
        SyncConfig.changeModeUpdate);
