...
import '../models/models_factory.dart';
...
class ExportImportService {
  ...
  Future<String> exportAllTablesToCsvZip() async {
    ...
    for (final tableName in DataDefinition.allDataTables) {
      try {
      ...
      for (int i = 1; i < rowsAsListOfValues.length; i++) {
        try {
          ...
          try {
            ...
            final modelInstance = ModelsFactory.fromMap(tableName, rawRecord);
            typedRecord = ModelsFactory.toMap(modelInstance, tableName);
          } catch (e) {
            // If model factory fails, fall back to heuristic parsing
            final errorMsg = 'Row ${i + 1}: ${e.toString().split('\n').first}';
            errorMessages.add(errorMsg);

            typedRecord = DataTypeUtils.convertRecordTypes(rawRecord, headers);
          }
      ...
      try {
        ...
        while (true) {
          ...
          final dataResponse = await sheetsApi.spreadsheets.values.get(
            sheetId,
            '$tableName!A$startRow:$lastColLetter$endRow',
          );
          ...
          for (final sheetRec in sheetDataBatch) {
            try {
              ...
              for (int i = 0; i < sheetColumnNames.length; i++) {
                ...
                final stringValue = cellValue?.toString().trim() ?? '';
                rawRecord[dbCol] = stringValue.isEmpty ? null : stringValue;
              }
              ...
              try {
                // Use the model's fromMap to get proper types, then convert back to map
                final modelInstance =
                    ModelsFactory.fromMap(tableName, rawRecord);
                typedRecord = ModelsFactory.toMap(modelInstance, tableName);
              } catch (e) {
                debugPrint(
                    '[Import] Model factory failed for row in $tableName: $e');
                // Fall back to heuristic parsing
                typedRecord = DataTypeUtils.convertRecordTypes(rawRecord);
              }
  ...
  Future<Map<String, num>> readDataStatisticsFromGoogleSheet({
    required String sheetId,
    required http.Client client,
  }) async {
    final sheetsApi = sheets.SheetsApi(client);
