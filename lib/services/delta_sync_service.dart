...
class DeltaSyncService {
    ...
    for (final change in batch) {
      ...
      if (changeMode != ChangeModes.delete) {
        ...
        final records = await db.query(
          tableName,
          where: "${DataDefinition.getKeyColumn(tableName)} = ?",
          whereArgs: [key],
          limit: 1,
        );
  ...
  Future<void> _upsertRecord(String table, Map<String, dynamic> record) async {
    ...
    final primaryKeyColumn = DataDefinition.getKeyColumn(table);
    ...
    if (existing.isEmpty) {
      ...
      final tableIndex = DataDefinition.getTableIndex(table);
      if (tableIndex != null) {
